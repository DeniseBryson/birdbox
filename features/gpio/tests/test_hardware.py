...
def mock_gpio():
    ...
    with patch('features.gpio.hardware.RPI_GPIO') as mock:
        ...
        # Set up gpio_function to return valid modes for test pins
        def gpio_function(pin: int) -> int:
            if pin in [18, 23, 24]:  # Our test pins
                return IN
            raise ValueError(f"Invalid pin {pin}")
        mock.gpio_function = Mock(side_effect=gpio_function)
...
def gpio_hardware(mock_gpio: Mock) -> GPIOHardware:
    ...
    # Pre-cache valid pins to avoid mock issues during tests
    hardware.get_valid_pins()
    return hardware
...
class TestGPIOManager:
    ...
    def test_get_valid_pins_success(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
        ...
        mock_gpio.gpio_function.side_effect = gpio_function_side_effect
        gpio_hardware._valid_pins = None  # Clear cache to force re-fetch
        
        valid_pins = gpio_hardware.get_valid_pins()
        assert valid_pins == [2, 3, 4, 17, 18, 27]
        
        # Test caching behavior
        mock_gpio.gpio_function.side_effect = None  # Clear side effect
        cached_pins = gpio_hardware.get_valid_pins()
        assert cached_pins == valid_pins  # Should return cached result
        
    def test_get_valid_pins_no_pins(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
        # Configure mock to return no valid pins
        mock_gpio.gpio_function.side_effect = ValueError("Invalid pin")
        gpio_hardware._valid_pins = None  # Clear cache to force re-fetch
        
        with pytest.raises(RuntimeError, match="No valid pins found"):
            gpio_hardware.get_valid_pins()

    def test_setup_input_pin_basic(self, gpio_hardware: GPIOHardware, mock_gpio: Mock, mock_logger: Mock):
        pin = 18
    ...
    def test_setup_input_pin_with_edge_detection(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
        pin = 18
        callback = Mock()
        bouncetime = 300
        ...
        args = mock_gpio.add_event_detect.call_args
        ...
        mock_gpio.input.return_value = HIGH
        ...
        # Test with LOW state
        callback.reset_mock()
        mock_gpio.input.return_value = LOW
        wrapped_callback(pin)  # Simulate edge detection again
        callback.assert_called_once_with(pin, LOW)

    def test_setup_input_pin_edge_detection_validation(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
    ...
    def test_setup_output_pin_basic(self, gpio_hardware: GPIOHardware, mock_gpio: Mock, mock_logger: Mock):
        pin = 18
    ...
    def test_setup_output_pin_with_initial_state(self, gpio_hardware: GPIOHardware, mock_gpio: Mock, mock_logger: Mock):
        pin = 18
    ...
    def test_setup_output_pin_invalid_state(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
        with pytest.raises(ValueError, match="Invalid initial state, must be HIGH or LOW"):
            gpio_hardware.setup_output_pin(18, initial_state=999) # type: ignore

    def test_set_output_state_success(self, gpio_hardware: GPIOHardware, mock_gpio: Mock, mock_logger: Mock):
        ...
        mock_gpio.output.assert_called_with(pin, HIGH)
        ...
        mock_gpio.output.assert_called_with(pin, LOW)
        mock_logger.info.assert_called_with(f"Set pin {pin} state to {LOW}")

    def test_set_output_state_invalid_pin(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
    ...
    def test_set_output_state_invalid_state(self, gpio_hardware: GPIOHardware):
        ...
        with pytest.raises(ValueError, match="Invalid initial state, must be HIGH or LOW"):
            gpio_hardware.set_output_state(pin, 999) # type: ignore

    def test_get_pin_state_success(self, gpio_hardware: GPIOHardware, mock_gpio: Mock):
        ...
        assert state == HIGH
        mock_gpio.input.assert_called_once_with(pin)
    ...
    def test_cleanup(self, gpio_hardware: GPIOHardware, mock_gpio: Mock, mock_logger: Mock):
