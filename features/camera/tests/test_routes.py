...
from features.camera.routes import camera_bp, camera_manager
...
def test_initialize_camera(client):
    """Test camera initialization endpoint"""
    with patch.object(camera_manager, 'initialize_camera', return_value=True):
...
def test_initialize_camera_failure(client):
    """Test camera initialization failure"""
    with patch.object(camera_manager, 'initialize_camera', return_value=False):
        response = client.post('/api/v1/camera/initialize/0')
...
def test_stop_camera(client):
    """Test camera stop endpoint"""
    with patch.object(camera_manager, 'stop_camera'):
